...
describe('translateAcpEvent — lifecycle', () => {
  it('drops exit/error events (AcpBridge re-keys them by ACP sessionId)', () => {
    expect(translateAcpEvent({ kind: 'exit', sessionId: 'h1', code: 1, expected: false } as AcpEvent)).toEqual([]);
    expect(translateAcpEvent({ kind: 'error', sessionId: 'h1', error: 'ENOENT' } as AcpEvent)).toEqual([]);
  });
});
