...
- **External links** — `setWindowOpenHandler` opens only `http:` / `https:`
  ...
  _Enforced._
- **Auto-update feed is fixed at build time.** `electron-updater`
  (`src/main/update/updater.ts`) reads the GitHub owner/repo from
  `app-update.yml`, generated by electron-builder's `publish:` config — the
  renderer cannot redirect it to another feed. macOS (Squirrel.Mac) also
  refuses to apply an update whose code signature doesn't match the running
  app's, so a tampered or unsigned artifact is rejected before install even
  if the feed itself were compromised. Downloading is automatic; installing
